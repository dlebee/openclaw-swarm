...
## No plan steps that only populate a cache

A plan step exists to do real work toward the user's goal. **Do not** add a step whose `Execute` is only a cache write — such a step will always render "will execute" in the plan tree even on a fully-converged system, which is noise. If a value needs memoisation for the rest of the run, put the memoisation behind an on-demand `getOrResolve*()` helper and call it from the step that actually consumes the value. See `mesh/control_url.go` and `mesh/preauth_key.go` for the shape.
...
Reference: `mesh/control_url.go:getOrResolveControlURL`.
